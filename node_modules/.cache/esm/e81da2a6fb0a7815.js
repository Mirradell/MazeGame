let GetRandomInt,Maze,DB;_fb0‍.x([["qToken",()=>qToken],["qStart",()=>qStart],["qTurn",()=>qTurn],["qFinish",()=>qFinish],["qDisconnect",()=>qDisconnect],["aStatistics",()=>aStatistics]]);_fb0‍.w("./Maze.mjs",[["GetRandomInt",["GetRandomInt"],function(v){GetRandomInt=v}],["Maze",["Maze"],function(v){Maze=v}]]);_fb0‍.w("./dbWork.mjs",[["*",null,function(v){DB=v}]]);



const randomColor = _fb0‍.t("require")('randomcolor');
let lastRoomTime = -1;
const roomDelta = 5;
...
const secondsDiff = (time) => {
    ...
    roomsInfoMap.get(lastRoomTime).get("players").set(token, new Map([["place", playerPlace], ["color", playerColor]]));
       ...
       const qTurn = (socket) => (token, finalPoint) => {
    for (let room of roomsInfoMap) {
       ...
       const qFinish = (socket) => (token, chests, enemies) => {
   // if (token !== null)
        DB.UpdateDBByToken(token, chests, enemies);
}

       const qDisconnect = (socket) => (token) => {
    DB.DropRowInDB(token);
}

       const aStatistics = (socket) => (token, games, chests, enemies) => {
